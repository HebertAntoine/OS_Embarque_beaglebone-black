...
static int initPWM(void)
{
  ...
  if(!cm_per_pwm) {
    ...
    iowrite16(0x80,cm_per_pwm); // Very Important This register start in freezed mode.
  ...
  if(!cm_per_pwm) {
    ...
    iowrite16(sregval|(AQ_CLEAR << CAU | AQ_SET << PRD ),cm_per_pwm);
  ...
  if(!cm_per_pwm) {
    ...
    iowrite16(0x0000,cm_per_pwm); //Reset 0x100
    sregval = ioread16(cm_per_pwm);
    printk("DUTYHR 0x%04x\n",sregval);
  }
