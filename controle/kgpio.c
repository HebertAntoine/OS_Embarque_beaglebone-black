...
#define OFFSET_PIN_9_42 0x964
#define GPIO0_7_PIN9_42 7
#define GPIO_PIN9_42 7
//89
...
static long char_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
  ...
  case PWM_SET :
    break;
  case GPIO_SET:
    gpio_set_value(GPIO_PIN9_42, 1);
    break;
  case GPIO_CLEAR:
    gpio_set_value(GPIO_PIN9_42, 0);
    break;
  default :
...
static int __init pinmuxGPIO(void)
{
 int ret=0;
  ...
  cm_per_gpio = ioremap(AM33XX_CONTROL_BASE+OFFSET_PIN_9_42,4);
  if(!cm_per_gpio) {
    printk("cm_per_gpio_reg ioremap: error\n");
  }
  else {
    regval = ioread32(cm_per_gpio) ; //get GPIO MUX register value
    iowrite32( 0x2<<3|0x7 ,cm_per_gpio);// p. 1426 : GPIO MUX to GPIO OUTPUT (Mode 7)
  ...
  ret = gpio_request_one( GPIO_PIN9_42 , GPIOF_OUT_INIT_HIGH,
			  DEVICE_NAME " gpio");
  if (ret < 0) {
...
static int __init kernmodex_init(void)
{
  ...
  if (ret < 0) {
    printk(KERN_WARNING "KGPIO: unable to get a major\n");
