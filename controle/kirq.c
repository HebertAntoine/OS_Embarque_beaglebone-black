...
#define OFFSET_PIN_8_12 0x830
#define GPIO1_12_PIN8_12 12

#define GPIO_PIN8_12 44
...
int counter=0;
int cookie=42;
static irqreturn_t test_irq_latency_interrupt_handler(unsigned long ptr, void* arg)
{
  counter++;
  printk( KERN_ALERT "irq handler counter increased : %d \n", counter);
  return IRQ_HANDLED;
}
...
static int __init pinmuxIRQ(void)
{
  ...
  cm_per_gpio = ioremap(AM33XX_CONTROL_BASE+OFFSET_PIN_8_12,4);
  if(!cm_per_gpio) {
    printk("cm_per_gpio_reg ioremap: error\n");
  }
  else {
    regval = ioread32(cm_per_gpio) ; //get GPIO MUX register value
    iowrite32( 0x7|(2<< 3)|(1<<5) ,cm_per_gpio);// p. 1426 : GPIO MUX to GPIO INPUT (Mode 7)
  ...
  ret = gpio_request_one( GPIO_PIN8_12, GPIOF_IN,
			  DEVICE_NAME " gpio");
  if (ret < 0) {
    printk(KERN_ALERT DEVICE_NAME " : failed to request GPIO pin.\n" );
    return ret;
  }
  ret  = gpio_to_irq(GPIO_PIN8_12);
  if (ret < 0) {
  ...
  //irq_disable_irq(irq);
  //irq_set_handler_locked(irq,(irq_flow_handler_t)test_irq_latency_interrupt_handler2);
  //irq_set_irq_type(irq,IRQ_TYPE_EDGE_FALLING);
  //enable_irq(irq);
  ret = request_any_context_irq(
				irq,
				(irq_handler_t)test_irq_latency_interrupt_handler,
				IRQF_SHARED | IRQ_TYPE_EDGE_FALLING ,
				DEVICE_NAME,
				(void*)test_irq_latency_interrupt_handler
				);
  if (ret < 0) {
...
static int __init kernmodex_init(void)
{
  ...
  printk(KERN_ALERT "Loading example driver by Ludo...\n");
  ...
  if (ret < 0) {
    printk(KERN_WARNING "KIRQ: unable to get a major\n");
    return ret;
  }
  if (major == 0)
...
static void __exit kernmodex_exit(void)
{
  ...
  free_irq(gpio_to_irq(GPIO_PIN8_12),(void*)test_irq_latency_interrupt_handler);
  gpio_free(GPIO_PIN8_12);
